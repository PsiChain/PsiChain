...
#include "psicrypto/include/psicrypto.hpp"
...
static const psicrypto::KeyType DEFAULT_KEYTYPE = psicrypto::KeyType::XMSS_256_H10;
...
{
	return psicrypto::is_xmss_signature(der_sig.data(), der_sig.size(), false);
}

inline bool xmss_is_der_pubkey(uint8_t const * pubkey, size_t pubkey_size) noexcept
{
	return psicrypto::is_xmss_pubkey(pubkey, pubkey_size);
}

inline bool xmss_is_der_pubkey(std::vector<uint8_t> const & pubkey) noexcept
{
	return psicrypto::is_xmss_pubkey(pubkey.data(), pubkey.size());
}
...
psicrypto::secure_vector<uint8_t> ecdsa_private_export_der(uint8_t const * key, size_t key_size, bool fCompressed);
...
psicrypto::secure_vector<uint8_t> ecdsa_import_der_privkey(
	psicrypto::secure_vector<uint8_t> const & der_privkey);

bool ecdsa_privkey_tweak_add(unsigned char * keyChild, 
	psicrypto::secure_vector<uint8_t> const & tweak);
