...
CKeyType CPubKey::GetKeyType() const
{
	...
	case HDR_XMSS_V1:
		{
			auto info = psicrypto::get_key_info(vch.data(), vch.size());
...
{
	if ( psicrypto::check_pubkey(data, size) )
	{
...
bool CPubKey::Verify(const uint8_t * msg, size_t msg_size, const std::vector<unsigned char>& vchSig) const 
{
	...
	if ( IsXMSS() )
	{
		return psicrypto::xmss_verify(msg, msg_size, vchSig, vch);
	}
	else
	{
		uint256 hash;
...
bool CPubKey::IsFullyValid() const 
{
	...
	if (IsXMSS())
	{
		return psicrypto::is_xmss_pubkey(vch.data(), size());
	}
	else
	{
