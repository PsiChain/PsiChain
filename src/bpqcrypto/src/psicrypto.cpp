...
namespace psicrypto {
    ...
    BPQCRYPTO_PUBLIC bool xmss_generate(BPQ_KEYTYPE keytype, uint8_t *pKey, size_t * pKeySize) noexcept
    {
        ...
        try
        {
            auto params = psicrypto::xmss_params_from_keytype(convert_keytype(keytype));

            size_t key_size = psicrypto::get_key_size(params);
            ...
            *pKeySize = key_size;
            ...
            auto key = psicrypto::xmss_generate(params);
    ...
    BPQCRYPTO_PUBLIC bool is_xmss_key(uint8_t const *pKey, size_t nKeySize) noexcept
    {
        ...
        try
        {
            auto params = psicrypto::xmss_get_key_params(pKey, nKeySize);

            return nKeySize == psicrypto::get_key_size(params); // || nKeySize == psicrypto::get_short_key_size(params);
        }
        catch (...)
        {
            return false;
    ...
    BPQCRYPTO_PUBLIC bool is_xmss_short_key(uint8_t const *pKey, size_t nKeySize) noexcept
    {
        ...
        try
        {
            ...
            size_t key_size = psicrypto::get_short_key_size(params);
    ...
    BPQCRYPTO_PUBLIC bool is_xmss_pubkey(uint8_t const *pKey, size_t nKeySize) noexcept
    {
        ...
        try
        {
            ...
            size_t pub_size = psicrypto::get_pubkey_size(params);
