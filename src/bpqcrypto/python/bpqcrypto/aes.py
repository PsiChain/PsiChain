...
_psicryptodll.aes_cipher_create.restype = ctypes.c_void_p
_psicryptodll.aes256_cipher_create.restype = ctypes.c_void_p
_psicryptodll.aes_cipher_free.argtypes = [ctypes.c_void_p]
_psicryptodll.aes_encrypt.argtypes = [
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.c_long,
    ctypes.c_char_p,
...
_psicryptodll.aes_decrypt.argtypes = [
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.c_long,
    ctypes.c_char_p,
...
class _BaseCryptor(object):
    ...
    def __init__(self, key, encrypt: bool, iv=None):
        ...
        if self.key_size == 16:
            self._cipher = _psicryptodll.aes_cipher_create(_key, _iv, encrypt)
        elif self.key_size == 32:
            self._cipher = _psicryptodll.aes256_cipher_create(_key, _iv, encrypt)
    ...
    def __del__(self):
        ...
        if self._cipher is not None:
            _psicryptodll.aes_cipher_free(self._cipher)
            self._cipher = None
...
class Encryptor(_BaseCryptor):
    ...
    def encrypt(self, m):
        ...
        if not _psicryptodll.aes_encrypt(self._cipher, _m, _m_size, _e, ctypes.byref(_e_size)):
            raise RuntimeError()
...
class Decryptor(_BaseCryptor):
    ...
    def decrypt(self, m):
        ...
        if not _psicryptodll.aes_decrypt(self._cipher, _m, _m_size, _e, ctypes.byref(_e_size)):
            raise RuntimeError()
