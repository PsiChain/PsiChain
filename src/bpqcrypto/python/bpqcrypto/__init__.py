...
if sys.platform == 'darwin':
    _libname = 'libpsicrypto.dylib'
elif sys.platform == 'win32':
    _libname = 'psicrypto.dll'
else:
    _libname = 'libpsicrypto.so'

_psicryptodll = ctypes.CDLL(os.path.join(os.path.dirname(__file__), _libname))


_psicryptodll.get_version.restype = ctypes.c_char_p

_version = str(_psicryptodll.get_version(), "ascii")
...
def is_xmss_pubkey(x:bytes):
    ...
    return _psicryptodll.is_xmss_pubkey(_x, ctypes.c_long(len(x))) != 0
...
def is_xmss_short_key(x:bytes):
    ...
    _x = ctypes.create_string_buffer(x)
    return _psicryptodll.is_xmss_short_key(_x, ctypes.c_long(len(x))) != 0
...
def is_xmss_key(x:bytes):
    ...
    _x = ctypes.create_string_buffer(x)
    return _psicryptodll.is_xmss_key(_x, ctypes.c_long(len(x))) != 0
...
def is_xmss_signature(x:bytes, strict:bool=True):
    ...
    _x = ctypes.create_string_buffer(x)
    return _psicryptodll.is_xmss_signature(_x, ctypes.c_long(len(x)), ctypes.c_bool(strict)) != 0
...
def xmss_get_long_key(key:bytes):
    ...
    key2_size = _psicryptodll.xmss_get_key_size(_key, _key_size)
    if key2_size == _key_size.value:
        return key

    _key2 = ctypes.create_string_buffer(key2_size)
    if not _psicryptodll.xmss_get_long_key(_key, _key_size, _key2, ctypes.c_long(key2_size)):
        raise RuntimeError()
...
def xmss_get_short_key(key:bytes):
    ...
    key2_size = _psicryptodll.xmss_get_short_key_size(_key, _key_size)
    if key2_size == _key_size.value:
        return key

    _key2 = ctypes.create_string_buffer(key2_size)
    if not _psicryptodll.xmss_get_short_key(_key, _key_size, _key2, ctypes.c_long(key2_size)):
        raise RuntimeError()
...
def _xmss_generate(keytype):
    ...
    _psicryptodll.xmss_generate(ctypes.c_int(keytype), None, ctypes.byref(key_size))

    if key_size.value == 0:
        raise NotImplementedError()

    key = ctypes.create_string_buffer(key_size.value)
    if not _psicryptodll.xmss_generate(ctypes.c_int(keytype), key, ctypes.byref(key_size)):
        raise RuntimeError()
...
def _xmss_generate_from_seed(keytype, sk_seed, prf_seed, pub_seed):
    ...
    key_size = ctypes.c_ulong()
    _psicryptodll.xmss_generate_from_seed(ctypes.c_int(keytype),
                                          None, None, None,
    ...
    key = ctypes.create_string_buffer(key_size.value)
    if not _psicryptodll.xmss_generate_from_seed(ctypes.c_int(keytype),
...
def xmss_estimate_signature_size(key):
    ...
    _key_size = ctypes.c_long(len(key))
    _key = ctypes.create_string_buffer(key)
    return _psicryptodll.xmss_get_signature_size(_key, _key_size)
...
def _xmss_sign(message:bytes, key:bytes, key_index:int):
    ...
    _key_size = ctypes.c_long(len(key))
    _key = ctypes.create_string_buffer(key)
    sign_size = _psicryptodll.xmss_get_signature_size(_key, _key_size)
    if sign_size == 0:
        raise RuntimeError()
    ...
    res = _psicryptodll.xmss_sign(_msg, _msg_size, _key, _key_size, ctypes.byref(_key_index), _sign, ctypes.byref(_sign_size))
    if res <= 0:
        raise RuntimeError("psicrypto.xmss_sign error: %d", res)
...
def xmss_verify(msg:bytes, sig:bytes, pubkey:bytes):
    ...
    return _psicryptodll.xmss_verify(_msg, _msg_size, _sig, _sig_size, _pub, _pub_size) != 0
...
def xmss_get_pubkey(key:bytes):
    ...
    _key_size = ctypes.c_long(len(key))
    _key = ctypes.create_string_buffer(key)
    pub_size = _psicryptodll.xmss_get_pubkey_size(_key, _key_size)
    if pub_size == 0:
        raise RuntimeError()

    _pub = ctypes.create_string_buffer(pub_size)
    _pub_size = ctypes.c_long(pub_size)
    if not _psicryptodll.xmss_get_pubkey(_key, _key_size, _pub, _pub_size):
        raise RuntimeError()
...
def Hash256_SHA2(msg:bytes):
    ...
    if not _psicryptodll.hash_sha256(_msg, _msg_size, _hash):
        raise RuntimeError()
...
def Hash256_SHAKE128(msg:bytes):
    ...
    _hash = ctypes.create_string_buffer(32)
    if not _psicryptodll.hash256_shake128(_msg, _msg_size, _hash):
        raise RuntimeError()
...
def Hash_SHAKE128(msg:bytes, output_bits):
    ...
    if not _psicryptodll.hash_shake128(_msg, _msg_size, ctypes.c_long(output_bits), _hash):
        raise RuntimeError()
...
def get_key_info(key):
    ...
    _key = ctypes.create_string_buffer(key)
    _keyinfo = _KeyInfo()
    _psicryptodll.get_key_info(_key, _key_size, ctypes.pointer(_keyinfo))
    return {
...
def get_sig_info(sig):
    ...
    _sig = ctypes.create_string_buffer(sig)
    _siginfo = _SigInfo()
    _psicryptodll.get_sig_info(_sig, _sig_size, ctypes.pointer(_siginfo))
    return {
