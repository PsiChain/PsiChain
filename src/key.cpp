...
void check_use_count(uint8_t const * key, size_t key_size, size_t use_count, size_t utxo_count)
{
    ...
    auto info = psicrypto::get_key_info(key, key_size);
...
{
	if ( psicrypto::is_xmss_short_key(data, size) )
	{
		keydata = psicrypto::xmss_get_long_key(secure_vector(data, data + size));
		fValid = true;
		fLegacy = false;
	}
	else if ( psicrypto::is_xmss_key(data, size) )
	{
		keydata.assign(data, data + size);
		fValid = true;
		fLegacy = false;
	}
	else if (psicrypto::is_ecdsa_key(data, size))
	{
...
void CKey::MakeNewKey(CKeyType ktype) 
{
	...
	else
	{
        ...
        try
        {
			keydata = psicrypto::xmss_generate(ktype);
...
psicrypto::secure_vector<uint8_t> CKey::raw_private_key_legacy() const
{
	if ( IsXMSS() )
		return {};
		
	psicrypto::secure_vector<uint8_t> k;
...
size_t CKey::short_size() const
{
	if ( IsXMSS() )
	{
		return psicrypto::xmss_get_short_key_size(keydata);
	} else
	{
		return size();
	}
...
psicrypto::secure_vector<uint8_t> CKey::raw_short_key() const
{
	if ( IsXMSS() )
	{
		return psicrypto::xmss_get_short_key(keydata);
	} else
	{
		psicrypto::secure_vector<uint8_t> k;
...
psicrypto::secure_vector<uint8_t> CKey::raw_private_key() const
{
	if ( IsXMSS() )
	{
		return keydata;
...
CPubKey CKey::GetPubKey() const 
{
	...
	if ( IsXMSS() )
	{
		return CPubKey(psicrypto::xmss_get_pubkey(keydata));
	}
	else
	{
...
bool CKey::Sign(const uint8_t * msg, size_t msg_size, std::vector<unsigned char>& vchSig, uint32_t test_case) const 
{
    ...
    if ( IsXMSS() )
	{
        try
        {
			...
			vchSig = psicrypto::xmss_sign(msg, msg_size, keydata.data(), keydata.size(), use_count);
...
bool CKey::SignHash(const uint256 &hash, std::vector<unsigned char>& vchSig, uint32_t test_case) const 
{
    ...
    if ( IsXMSS() )
	{
        ...
        vchSig = psicrypto::xmss_sign(&hash.begin()[0], hash.size(),
