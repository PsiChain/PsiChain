...
WitnessV1ScriptHash::WitnessV1ScriptHash(const CScript& script)
   : uint256(psicrypto::hash256_shake128(&script.begin()[0], script.size()))
{
}
        ...
        txnouttype& typeRet, std::vector<std::vector<unsigned char> >& vSolutionsRet)
{
    ...
    for (const std::pair<txnouttype, CScript>& tplate : mTemplates)
    {
        ...
        while (true)
        {
            ...
            {
                while (psicrypto::check_pubkey(vch1.data(), vch1.size()))
                {
            ...
            {
                if (!psicrypto::check_pubkey(vch1.data(), vch1.size()))
                    break;
                vSolutionsRet.push_back(vch1);
            }
...
CTxDestination GetDestinationForWitness(const CScript& redeemscript)
{
    ...
    if (Solver(redeemscript, typ, vSolutions)) 
    {
        if (typ == TX_PUBKEY) 
        {
            if (psicrypto::is_xmss_pubkey(vSolutions[0]))
            {
        ...
        else if (typ == TX_MULTISIG)
        {
            ...
            if (pub_N + 2 == (int)vSolutions.size())
            {
                ...
                {
                    if (psicrypto::is_xmss_pubkey(vSolutions[i+1]))
                        ++v1_n;
                    else
                    if (psicrypto::is_ecdsa_pubkey(vSolutions[i+1]))
                        ++v0_n;
                }
...
int DetectScriptVersion(const CScript& redeemscript)
{
    ...
    if (Solver(redeemscript, typ, vSolutions)) 
    {
        if (typ == TX_PUBKEY)
        {
            ...
            else if (psicrypto::is_ecdsa_pubkey(vSolutions[0]))
                return 0;
        }
        else if (typ == TX_MULTISIG)
        {
            ...
            {
                if (psicrypto::is_xmss_pubkey(vSolutions[i]))
                    ++count_v1;

                else if (psicrypto::is_ecdsa_pubkey(vSolutions[i]))
                    ++count_v0;
            }
